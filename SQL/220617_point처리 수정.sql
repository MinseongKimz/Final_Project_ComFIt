...
--> 기존 포인트가 안되던 문제 해결
--> DIRECT_PRODUCT의 SDATE~EDATE가 6월 10~15일로 되어있는데
--> 제안이 20일로 들어와서 문제가 됨.
--> SDATE~EDATE 사이의 값만 들어올 수 있게 조정을 해야할듯.

UPDATE DIRECT_PRODUCT
SET PD_HOPE_SDATE = TO_DATE('2022-06-17', 'YYYY-MM-DD')
, PD_HOPE_EDATE = TO_DATE('2022-06-20', 'YYYY-MM-DD')
WHERE DIRE_PD_ID = 'dire_1';

--> dire_1의 범위를 바꿔줌으로 해결.
--> EDATE가 다시 지나가면 포인트가 돌아오는것을 확인함.
COMMIT;
